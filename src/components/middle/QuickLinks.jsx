...
import { getTransactions, getCourses, getResults } from "../middle/services/Api";

const links = [
  { id: 1, label: 'Payment History', icon: <CreditCard className="w-5 h-5" />, dropdown: 'payment' },
  { id: 2, label: 'Courses', icon: <BookOpen className="w-5 h-5" />, dropdown: 'courses' },
  { id: 3, label: 'Results', icon: <FileText className="w-5 h-5" />, dropdown: 'results' },
];
          ...
          <li key={link.id} className='flex flex-col gap-2'>
            ...
            {openDropdown === link.id && link.dropdown === 'courses' && (
              <div className='ml-6 mt-2 bg-gray-50 p-3 rounded-lg border flex flex-col gap-2'>
                {courses.map((course) => (
                  <span key={course.id} className='text-gray-700 p-2 bg-gray-100 rounded hover:bg-gray-200'>
